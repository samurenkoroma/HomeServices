...
func toCropPlanDTO(plan *cropplan.CropPlan) *dto.CropPlanDTO {
	...
	return &dto.CropPlanDTO{
		ID:           plan.ID(),
		BedID:        plan.Area().GetId(),
		Name:         plan.Name(),
		VarietyID:    plan.Variety().GetId(),
		VarietyName:  plan.Variety().GetName(),
		CropName:     plan.Variety().GetSpeciesName(),
		Status:       string(plan.Status()),
		StatusText:   getStatusText(plan.Status()),
		SeasonStart:  plan.Season().GetStartDate(),
		SeasonEnd:    plan.Season().GetEndDate(),
