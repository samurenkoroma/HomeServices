...
func NewCropRepo(tx *sql.Tx) domain2.CropPlanRepository {
...
func (r *CropPlanRepoImp) ByID(
	ctx context.Context,
	id domain2.CropPlanID,
) (*domain2.CropPlan, error) {
	...
	plan, err := domain2.NewCropPlan(
		domain2.CropPlanID(planID),
		domain2.CropTypeID(cropTypeID),
		name,
		duration,
	)
	if err != nil {
	...
	plan.Rehydrate(version, domain2.CropPlanStatus(status))
