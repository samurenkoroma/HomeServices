...
func NewCropPlan(id CropPlanID, cropType CropTypeID, name string, duration int) (*CropPlan, error) {
	...
	plan.AddEvent(NewCropPlanCreated(string(id), string(cropType)))
	return plan, nil
}
...
func (c *CropPlan) AddStage(stage GrowthStage) error {
	...
	c.AddEvent(NewGrowthStageAdded(string(c.id), stage.Order(), stage.Name()))
	return nil
}
...
func (c *CropPlan) Publish() error {
	...
	c.AddEvent(NewCropPlanPublished(string(c.id), c.version))
	return nil
}
